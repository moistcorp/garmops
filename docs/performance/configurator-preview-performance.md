...
The photographic render ceiling remains 3000 px because no visual evidence justified reducing it. Asset normalization removes unused colour entropy while preserving mask alpha, rounded luminance, dimensions and framing. Transfer totals are recorded in the asset optimization report. Long immutable cache headers are applied to `/garments/*`; changing renderer assets requires a deployment/version change.
