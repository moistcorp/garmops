...
export default async function StaffQuotePaymentPage({ params }: { params: Promise<{ token: string }> }) {
      ...
      <div className="grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
        <section className="techpack-surface rounded border p-6 sm:p-8">
          ...
          <div className="mt-6 border-t border-black/10 pt-5 text-sm">
            ...
            <div className="mt-2 flex justify-between"><span>GST at 5%</span><strong>{formatMoneyPaise(quote.taxPaise)}</strong></div>
        ...
        <aside className="techpack-surface h-fit rounded border p-6"><h2 className="font-semibold">Review and pay</h2><p className="mt-2 text-xs leading-relaxed text-black/50">You are signed in with the quoted customer email. Complete full payment securely through PayU.</p><div className="mt-5"><StaffQuoteCheckout token={token} totalPaise={quote.totalPaise} /></div></aside>
      </div>
    </main>
  );
}
