...
export default async function FoundryPortalLayout({ children }: { children: React.ReactNode }) {
  ...
  const nav = [{ href: "/orders", label: "Orders" }, { href: "/artwork-review", label: "Artwork review" }, { href: "/payments", label: "Payments" }, ...(staff.role === "founder" ? [{ href: "/analytics", label: "Analytics" }, { href: "/staff-management", label: "Staff" }] : [])];
