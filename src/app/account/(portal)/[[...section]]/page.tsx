      ...
      <section className="liquid-glass-surface rounded-3xl border p-6">
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-2">
          {files?.map((file) => {
            ...
            return <article key={file.id} className="rounded-2xl border border-black/8 bg-white/45 p-5"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="truncate text-sm font-semibold">{file.original_filename}</p><p className="mt-1 text-xs capitalize text-black/45">{file.kind.replaceAll("_", " ")} · {order.order_number}</p></div><InvoiceDownloadButton fileId={file.id} /></div></article>;
          })}
...
async function NotificationsPage() {
  ...
  return <div className="space-y-5"><section className="liquid-glass-surface rounded-3xl border p-6 sm:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><Bell size={18} className="text-[#4F8B92]" /><h2 className="text-xl font-semibold">Notifications</h2></div><p className="mt-2 text-sm text-black/50">Approval requests, customer actions, status updates, and shipment events appear here.</p></div>{unread ? <form action={markAllNotificationsReadAction}><button className="rounded-full border border-black/10 bg-white/60 px-4 py-2 text-xs font-semibold">Mark all read</button></form> : null}</div></section><div className="space-y-3">{notifications?.map((notification) => <article key={notification.id} className={`rounded-2xl border p-5 ${notification.read_at ? "border-black/8 bg-white/45" : "border-[#4F8B92]/35 bg-[#4F8B92]/8"}`}><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-semibold">{notification.title}</p><p className="mt-2 text-sm leading-relaxed text-black/55">{notification.body}</p><p className="mt-2 text-[10px] uppercase tracking-wider text-black/30">{formatOrderDate(notification.created_at)}</p></div><div className="flex gap-2">{notification.action_url ? <Link href={notification.action_url} className="rounded-full bg-[#16212B] px-4 py-2 text-xs font-semibold text-white">Open</Link> : null}{!notification.read_at ? <form action={markNotificationReadAction}><input type="hidden" name="notificationId" value={notification.id} /><button className="rounded-full border border-black/10 bg-white px-4 py-2 text-xs font-semibold">Mark read</button></form> : null}</div></div></article>)}{!notifications?.length ? <PortalPlaceholder title="No notifications" description="New approval, order, and shipment updates will appear here." /> : null}</div></div>;
}
...
async function AccountOverview() {
  ...
  return <div className="space-y-5"><section className="liquid-glass-surface rounded-3xl border p-6 sm:p-8"><h2 className="text-xl font-semibold">Company operations overview</h2><p className="mt-2 text-sm text-black/50">Orders needing action, versioned approvals, shipments, notifications, and reusable historical configurations are collected here.</p></section><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[["Active orders",active],["Approvals due",approvals ?? 0],["Unread updates",unread ?? 0],["Documents",documents ?? 0]].map(([label,value]) => <div key={String(label)} className="liquid-glass-panel rounded-2xl border p-5"><p className="text-[10px] uppercase tracking-wider text-black/35">{label}</p><p className="mt-2 text-2xl font-semibold">{value}</p></div>)}</div><section className="liquid-glass-surface rounded-3xl border p-6"><div className="flex items-center justify-between gap-4"><h3 className="font-semibold">Recent orders</h3><Link href="/account/orders" className="text-xs font-semibold text-[#315F66] hover:underline">View all</Link></div><div className="mt-5 space-y-3">{orders?.map((order) => <Link key={order.id} href={`/account/orders/${order.order_number}`} className="flex items-center justify-between gap-4 rounded-2xl border border-black/8 bg-white/45 p-4 transition hover:border-[#4F8B92]/40"><div><p className="text-sm font-semibold">{order.order_number}</p><p className="mt-1 text-xs text-black/45">{order.customer_reference ?? "Custom order"}</p></div><span className="text-xs capitalize text-black/50">{order.public_status.replaceAll("_", " ")}</span></Link>)}{!orders?.length ? <p className="text-sm text-black/40">No orders yet.</p> : null}</div>{delivered ? <p className="mt-4 text-xs text-black/45">Delivered orders include a reorder action that creates a fresh price and a new order number.</p> : null}</section></div>;
}
