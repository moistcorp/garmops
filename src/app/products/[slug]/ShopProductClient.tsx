...
function MeasurementGuide({ product, sizeChart }: { product: Product; sizeChart: SizeChart }) {
  ...
  return (
    <div className="techpack-panel rounded-sm border p-5 sm:p-6">
      ...
      <div className="mt-5 space-y-3 text-sm leading-6 text-(--text-primary)/60">
        <p><span className="font-semibold text-(--text-primary)">Chest:</span> use the chest value exactly as shown for this specific style.</p>
        {hasMeasurement(sizeChart, 'length') && <p><span className="font-semibold text-(--text-primary)">Length:</span> vertical garment length from the upper body to the lower hem.</p>}
        {hasShoulder && <p><span className="font-semibold text-(--text-primary)">Shoulder:</span> shoulder measurement shown in the approved chart.</p>}
...
function SimpleSizeChart({ chart }: { chart: SizeChart }) {
  ...
  return (
    <div>
      {chart.fitNote && (
        <div className="mb-4 inline-flex rounded-[3px] bg-(--color-cream-soft) px-3 py-2 text-xs font-medium text-(--text-primary)/65">
          {chart.fitNote}
        </div>
      )}

      <div className="techpack-panel overflow-hidden rounded-sm border">
        <div className="overflow-x-auto" role="region" aria-label="Product size chart" tabIndex={0}>
          <table className="w-full min-w-[420px] text-sm">
            ...
            <tbody className="divide-y divide-[#E5E0D8]">
              {chart.sizes.map(row => (
                <tr key={row.size}>
                  <td className="px-4 py-3 font-semibold text-(--text-primary)">{displaySize(row.size)}</td>
                  {showChest && <td className="px-4 py-3 text-(--text-primary)/65">{row.chest ?? '—'}</td>}
                  {showLength && <td className="px-4 py-3 text-(--text-primary)/65">{row.length ?? '—'}</td>}
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {extraColumns.length > 0 && (
        <details className="group mt-3 rounded-sm border border-[#E5E0D8] bg-white/30">
              ...
              <tbody className="divide-y divide-[#E5E0D8]">
                {chart.sizes.map(row => (
                  <tr key={row.size}>
                    ...
                    {showLength && <td className="px-4 py-3 text-(--text-primary)/65">{row.length ?? '—'}</td>}
                    {extraColumns.map(column => (
                      <td key={column.key} className="px-4 py-3 text-(--text-primary)/65">{renderCell(row, column.key)}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
      ...
      <main className="mx-auto max-w-7xl px-4 pb-16 pt-10 sm:px-6 sm:pb-24 sm:pt-16">
        ...
        <section className="grid gap-8 sm:gap-12 md:grid-cols-2 md:gap-16">
          <div className="relative">
            {product.image ? (
              <div className="relative aspect-[3/4] overflow-hidden rounded-sm border border-[#ECE7DF]">
                <Image
                  src={product.image}
          ...
          <div className="flex flex-col justify-center py-1 md:py-6">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              <span className="rounded-[3px] bg-[#F1EEE8] px-3 py-1.5 text-xs font-medium text-(--text-primary)/65">{product.gsm} GSM</span>
              <span className="rounded-[3px] bg-[#F1EEE8] px-3 py-1.5 text-xs font-medium text-(--text-primary)/65">{product.selectorMaterial}</span>
              <span className="rounded-[3px] bg-[#F1EEE8] px-3 py-1.5 text-xs font-medium text-(--text-primary)/65">{productFitLabel(product)}</span>
            </div>
              ...
              <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2">
                {product.bestFor.map(item => (
                  <span key={item} className="inline-flex items-center gap-1.5 text-sm text-(--text-primary)/65">
                    <Check size={13} className="text-(--color-accent)" /> {item}
                  </span>
                ))}
              </div>
            </div>
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              <Link
                ...
                className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-sm bg-(--color-accent) px-6 py-3 text-sm font-semibold text-white transition hover:bg-(--color-accent-dark)"
              >
                Customise this {product.selectorCategory === 'Tote Bags' ? 'tote' : 'product'} <ArrowRight size={15} />
              </Link>
              <a
                href="#order-sample"
                className="inline-flex min-h-12 flex-1 items-center justify-center rounded-sm border border-[#D7D1C7] px-6 py-3 text-sm font-semibold text-(--text-primary) transition hover:border-(--text-primary)/35"
              >
                Order a sample
              </a>
            </div>
        ...
        <section className="mt-16 border-t border-[#E5E0D8] pt-12 sm:mt-24 sm:pt-16" aria-labelledby="why-choose-title">
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-3">
            {benefits.map((benefit, index) => (
              <article key={benefit.title} className="techpack-panel rounded-sm border p-5 sm:p-6">
                <span className="text-[10px] font-semibold tracking-[0.16em] text-(--color-accent)">0{index + 1}</span>
                <h3 className="mt-4 text-base font-semibold text-(--text-primary)">{benefit.title}</h3>
                <p className="mt-2 text-sm leading-6 text-(--text-primary)/60">{benefit.description}</p>
              </article>
            ))}
          </div>
        </section>
          ...
          <div>
            ...
            <div className="mt-6 space-y-3">
              {decorationMethods.map(method => (
                <article key={method.name} className="techpack-panel rounded-sm border p-4 sm:p-5">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-sm font-semibold text-(--text-primary)">{method.name}</h3>
                    {method.recommended && (
                      <span className="rounded-[3px] bg-(--color-accent)/10 px-2 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-(--color-accent-dark)">Recommended</span>
                    )}
                  </div>
                  <p className="mt-2 text-sm leading-6 text-(--text-primary)/60">{method.description}</p>
                </article>
              ))}
            </div>
          </div>
            ...
            <div className="techpack-panel rounded-sm border p-5 sm:p-6">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {product.sizes.map(size => (
                      <button
                        ...
                        className={`min-h-11 min-w-12 rounded-sm border px-3 text-sm font-medium transition-colors ${selectedSize === size ? 'techpack-selected' : 'techpack-control text-(--text-primary) hover:!border-(--color-accent) hover:text-(--color-accent)'}`}
                      >
                        {displaySize(size)}
                      </button>
                    ))}
              ...
              <div className="mt-6 border-t border-[#E5E0D8] pt-5">
                ...
                <div className="mt-3 flex items-center gap-3">
                  ...
                  <button type="button" onClick={() => setQuantity(value => Math.min(MAX_SAMPLE_ITEM_QUANTITY, value + 1))} disabled={quantity >= MAX_SAMPLE_ITEM_QUANTITY} className="techpack-control flex h-10 w-10 items-center justify-center rounded-sm border text-lg disabled:cursor-not-allowed disabled:opacity-40">+</button>
                </div>
              </div>

              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                <button type="button" onClick={handleAdd} className="min-h-12 rounded-sm border border-(--color-accent) px-5 py-3 text-sm font-semibold text-(--color-accent) transition hover:bg-(--color-accent) hover:text-white">
                  {added ? 'Added to sample cart' : 'Add sample to cart'}
                </button>
                <button type="button" onClick={handleBuyNow} className="min-h-12 rounded-sm bg-(--text-primary) px-5 py-3 text-sm font-semibold text-white transition hover:opacity-90">
                  Buy sample now
                </button>
              </div>
            </div>
        ...
        <section className="mt-16 overflow-hidden rounded-sm border border-[#E5E0D8] bg-(--color-cream-soft) px-5 py-8 sm:mt-20 sm:px-8 sm:py-10 lg:px-10" aria-labelledby="make-it-yours-title">
          ...
          <Link href={configuratorHref} className="mt-7 inline-flex min-h-12 items-center justify-center gap-2 rounded-sm bg-(--color-accent) px-6 py-3 text-sm font-semibold text-white transition hover:bg-(--color-accent-dark)">
        ...
        <section className="mt-12 border-t border-[#E5E0D8] pt-8 sm:mt-16">
          <details className="group border-b border-[#E5E0D8]">
            <summary className="flex cursor-pointer list-none items-center justify-between py-4 text-sm font-semibold text-(--text-primary) [&::-webkit-details-marker]:hidden">
              Care instructions
              <ChevronDown size={16} className="transition-transform group-open:rotate-180" />
            </summary>
            <ul className="grid gap-2 pb-5 sm:grid-cols-2">
              {product.careInstructions.map(instruction => (
                <li key={instruction} className="text-sm text-(--text-primary)/60">— {instruction}</li>
              ))}
            </ul>
          </details>
        </section>
