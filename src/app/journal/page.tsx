  ...
  return (
    <div className="techpack-canvas">
      <section className="max-w-7xl mx-auto px-4 pb-10 pt-10 sm:px-6 sm:pb-16 sm:pt-20">
        <p className="text-xs text-[var(--text-primary)]/40 font-medium mb-4 tracking-widest uppercase">Guides</p>
        <h1 className="text-4xl sm:text-5xl font-bold text-[var(--text-primary)] max-w-2xl leading-tight mb-6 tracking-tight">Custom apparel &amp; merchandise guides</h1>
        <p className="max-w-2xl text-base leading-relaxed text-[var(--text-primary)]/50 sm:text-lg">Clear, production-led answers about bulk T-shirt printing, garment specifications, decoration methods, pricing, MOQs and planning branded merchandise in India.</p>
      </section>
          ...
          {posts.slice(1).map(post => (
            <Link key={post.slug} href={`/journal/${post.slug}`}
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs border border-[#ECE7DF] rounded-[4px] px-2.5 py-1 text-[var(--text-primary)]/50">{post.category}</span>
                <span className="text-xs text-[var(--text-primary)]/30">{post.readTime}</span>
              </div>
              <h3 className="text-sm font-semibold text-[var(--text-primary)] leading-snug group-hover:underline">{post.title}</h3>
              <p className="text-xs text-[var(--text-primary)]/50 leading-relaxed flex-1">{post.excerpt}</p>
              <time dateTime={post.publishedAt} className="text-xs text-[var(--text-primary)]/30">{post.date}</time>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="techpack-section py-12 sm:py-16">
        <div className="max-w-7xl mx-auto flex flex-col items-stretch justify-between gap-6 px-4 sm:px-6 md:flex-row md:items-center">
          <div>
            ...
            <p className="text-[var(--text-primary)]/50 text-sm">50 pieces minimum. Quote within 24 hours.</p>
          </div>
