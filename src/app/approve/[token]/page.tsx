...
export default async function ExternalApprovalPage({
  ...
  return <main className="min-h-screen bg-[#eef3f2] px-4 py-12 text-[#16212B]">
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      {completed ? <section className="rounded-[4px] border border-emerald-200 bg-emerald-50 p-6"><div className="flex items-center gap-2 font-semibold text-emerald-800"><CheckCircle2 size={19} /> Decision recorded</div><p className="mt-2 text-sm text-emerald-800/75">This version was {decision}. Download the PDF while this secure link remains available if you need a copy for your records.</p></section> : active ? <section className="rounded-[4px] border border-[var(--color-rule)] bg-white p-6   sm:p-9"><form action={externalApprovalResponseAction} className="space-y-4"><input type="hidden" name="token" value={token} /><label className="block text-sm font-medium">Decision<select name="decision" className="mt-2 w-full rounded-[4px] border border-black/10 bg-white px-3 py-3"><option value="approved">Approve this exact version</option><option value="changes_requested">Request changes</option></select></label><label className="block text-sm font-medium">Note<textarea name="responseNote" rows={4} className="mt-2 w-full rounded-[4px] border border-black/10 bg-white px-3 py-3" placeholder="Add a clear reason when requesting changes." /></label><button className="rounded-[4px] bg-[#16212B] px-5 py-3 text-sm font-semibold text-white">Submit secure decision</button></form>{result === "error" ? <p className="mt-3 text-sm text-red-700">The decision could not be recorded. The link may have expired or already been used.</p> : null}</section> : <section className="rounded-[4px] border border-amber-200 bg-amber-50 p-6"><p className="font-semibold text-amber-900">This approval link is no longer active.</p><p className="mt-2 text-sm text-amber-800">Ask the Garmops team for a new request if another decision is required.</p></section>}
    </div>
  </main>;
}
