...
import { ArrowLeft, Clock3, FileText, MapPin, Palette, ReceiptIndianRupee, Ruler, Truck, UserRound } from "lucide-react";
...
import { CancellationDecisionForm, CancellationRequestForm, RefundForm, StatusTransitionForm } from "@/components/staff/StaffOrderForms";
...
export default async function StaffOrderWorkspace({ orderNumber }: { orderNumber: string }) {
  ...
  const [itemsResult, historyResult, filesResult, artworkRequirementsResult, invoiceResult, paymentResult, cancellationResult] = await Promise.all([
  ...
  const nextStatuses = allowedNextStatusesForRole(order.status as OrderStatus, context.role, { holdFromStatus: order.hold_from_status as OrderStatus | null, cancellationPending: Boolean(pendingCancellation) });
  const customerName = stringValue(customer, "name", "fullName", "contactName") || stringValue(billing, "contactName") || "Customer";

  return (
    <div className="space-y-6">
      <Link href="/orders" className="inline-flex items-center gap-2 text-sm font-medium text-black/50 hover:text-(--color-accent)"><ArrowLeft size={15} />Back to order queue</Link>
      <section className="techpack-surface rounded-sm border p-6 sm:p-8">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
        <div className="space-y-5">
          <section className="techpack-surface rounded-sm border p-6"><div className="flex items-center gap-2"><Palette size={18} className="text-(--color-accent)" /><h2 className="font-semibold">Paid order specification</h2></div><div className="mt-5 space-y-4">{items.map((item) => { const colour = record(item.colour_snapshot); const sizes = record(item.size_breakdown); const product = record(item.product_snapshot); const decoration = record(item.decoration_snapshot); return <article key={item.id} className="rounded-sm border border-black/7 bg-white p-4"><div className="flex flex-wrap justify-between gap-3"><div><h3 className="font-semibold">Line {item.line_number}: {item.product_name}</h3><p className="mt-1 text-sm text-black/50">{String(colour.name ?? colour.label ?? "Colour to review")} · {item.quantity.toLocaleString("en-IN")} units</p><p className="mt-1 text-xs text-black/40">{String(product.gsm ?? "")} {String(product.fit ?? "")} · {String(decoration.technique ?? decoration.frontTechnique ?? "Technique in configuration")}</p></div><span className="text-sm font-semibold">{formatMoneyPaise(item.line_total_paise)}</span></div><div className="mt-4 flex gap-2 border-t border-black/7 pt-4"><Ruler size={15} className="text-(--color-accent)" /><p className="text-xs text-black/55">{Object.entries(sizes).map(([size, quantity]) => `${size}: ${quantity}`).join(" · ")}</p></div></article>; })}</div></section>

          <section className="techpack-surface rounded-sm border p-6"><div className="flex items-center gap-2"><FileText size={18} className="text-(--color-accent)" /><h2 className="font-semibold">Private files</h2></div><div className="mt-5 space-y-3">{files.length ? files.map((file) => { const requirement = artworkRequirementByFile.get(file.id); const safeScan = file.scan_status === "clean" || file.scan_status === "not_required"; const scanLabel = file.scan_status === "clean" ? "Clean" : file.scan_status === "not_required" ? "Not required" : ["pending", "pending_scan", "manual_review"].includes(file.scan_status) ? "Scanning" : file.scan_status === "infected" ? "Infected" : ["scan_failed", "scanner_unavailable"].includes(file.scan_status) ? "Scan failed" : file.scan_status.replaceAll("_", " "); return <div key={file.id} className={`rounded border p-3 ${requirement && !requirement.is_active ? "border-black/5 bg-black/[0.02] opacity-65" : "border-black/7 bg-white"}`}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium">{file.safe_filename ?? file.original_filename}</p><p className="mt-1 text-xs capitalize text-black/45">{file.kind.replaceAll("_", " ")} · {file.content_type} · review {file.review_status}{requirement ? ` · revision ${requirement.revision} · ${requirement.is_active ? "active" : "superseded"}` : ""}</p>{file.kind === "customer_artwork" ? <p className={`mt-1 text-xs font-semibold ${safeScan ? "text-emerald-700" : file.scan_status === "infected" ? "text-red-700" : "text-amber-700"}`}>Malware scan: {scanLabel}{!safeScan ? " · production remains blocked" : ""}</p> : null}</div><PrivateFileDownloadButton fileId={file.id} /></div>{file.kind === "customer_artwork" && requirement?.is_active && file.review_status !== "approved" ? <div className="mt-3 border-t border-black/10 pt-3"><ArtworkReviewForm fileId={file.id} scanStatus={file.scan_status} /></div> : null}</div>; }) : <p className="text-sm text-black/45">No files attached.</p>}</div></section>

          <section className="techpack-surface rounded-sm border p-6"><div className="flex items-center gap-2"><Clock3 size={18} className="text-(--color-accent)" /><h2 className="font-semibold">Order timeline</h2></div><div className="mt-5 space-y-4">{history.map((entry) => <div key={entry.id} className="border-l border-black/10 pl-4"><p className="text-sm font-semibold">{ORDER_STATUS_LABELS[entry.to_status as OrderStatus] ?? entry.to_status}</p><p className="mt-1 text-xs text-black/50">{entry.customer_message ?? publicOrderStatusLabel(entry.public_status)}</p>{entry.internal_note ? <p className="mt-1 text-xs text-amber-800">Internal: {entry.internal_note}</p> : null}{entry.reason ? <p className="mt-1 text-xs text-black/45">Reason: {entry.reason}</p> : null}<p className="mt-2 text-xs uppercase tracking-wider text-black/30">{formatOrderTimestamp(entry.created_at)} · {entry.actor_type}</p></div>)}</div></section>
        </div>

        <div className="space-y-5">
          <section className="techpack-surface rounded-sm border p-6"><h2 className="font-semibold">Change order status</h2><div className="mt-5"><StatusTransitionForm orderId={order.id} orderNumber={order.order_number} currentStatus={order.status} nextStatuses={nextStatuses} /></div></section>
