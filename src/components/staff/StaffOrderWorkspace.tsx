...
import { AdministrativeOrderNotesForm, CancellationDecisionForm, CancellationRequestForm, ConfigurationRevisionForm, RefundForm, ReopenConfigurationForm, ShippingPaymentForm, StatusTransitionForm } from "@/components/staff/StaffOrderForms";
...
export default async function StaffOrderWorkspace({ orderNumber }: { orderNumber: string }) {
  ...
  const physicalProductionStatuses = ["material_preparation","printing_embroidery","stitching","quality_check","packing","ready_to_dispatch","dispatched","delivered"];
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
        <div className="space-y-5">
          ...
          <section className="techpack-surface rounded-[4px] border p-6"><div className="flex items-center gap-2"><FileText size={18} className="text-[var(--color-accent)]" /><h2 className="font-semibold">Private files</h2></div><div className="mt-5 space-y-3">{files.length ? files.map((file) => { const requirement = artworkRequirementByFile.get(file.id); return <div key={file.id} className={`rounded border p-3 ${requirement && !requirement.is_active ? "border-black/5 bg-black/[0.02] opacity-65" : "border-black/7 bg-white"}`}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium">{file.safe_filename ?? file.original_filename}</p><p className="mt-1 text-xs capitalize text-black/45">{file.kind.replaceAll("_", " ")} · {file.content_type} · review {file.review_status}{requirement ? ` · revision ${requirement.revision} · ${requirement.is_active ? "active" : "superseded"}` : ""}</p></div><PrivateFileDownloadButton fileId={file.id} /></div>{file.kind === "customer_artwork" && requirement?.is_active && file.review_status !== "approved" ? <div className="mt-3 border-t border-black/10 pt-3"><ArtworkReviewForm fileId={file.id} /></div> : null}</div>; }) : <p className="text-sm text-black/45">No files attached.</p>}</div></section>
