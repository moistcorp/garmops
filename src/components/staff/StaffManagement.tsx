...
export default async function StaffManagement() {
  ...
  return <div className="space-y-5"><TechpackPageHeader eyebrow="Founder" reference="Access control" title="Staff management" description="Every staff email is reserved as staff-only and must complete TOTP before Foundry access." /><div className="grid gap-5 xl:grid-cols-[0.75fr_1.25fr]"><section className="techpack-surface rounded border p-5"><h2 className="font-semibold">Invite staff</h2><div className="mt-4"><StaffManagementForms mode="invite" /></div></section><section className="techpack-surface rounded border p-5"><h2 className="font-semibold">Active directory</h2><div className="mt-4 divide-y divide-black/10">{staff.map((member) => { const profile = Array.isArray(member.profiles) ? member.profiles[0] : member.profiles; return <div key={member.user_id} className="flex flex-wrap items-center justify-between gap-3 py-4"><div><p className="font-semibold">{profile?.first_name} {profile?.last_name}</p><p className="text-xs text-black/45">{member.email} · {member.role} · MFA {member.mfa_enrolled_at ? "enrolled" : "pending"}</p></div><StaffManagementForms mode="toggle" userId={member.user_id} active={member.active} /></div>; })}</div></section></div></div>;
}
