...
export function ApprovalRequestForm({ orderId, orderNumber, files, approvers, designVersions }: {
  orderId: string;
  orderNumber: string;
  ...
  return <form action={action} className="space-y-3 rounded-[4px] border border-black/8 bg-white p-4">
    ...
    <label className="block text-xs font-medium">Expires<input type="datetime-local" name="expiresAtLocal" defaultValue={expiry} className={`${input} mt-1`} onChange={(e) => { const hidden = e.currentTarget.form?.elements.namedItem("expiresAt") as HTMLInputElement | null; if (hidden) hidden.value = new Date(e.currentTarget.value).toISOString(); }} /></label>
    <input type="hidden" name="expiresAt" defaultValue={new Date(expiry).toISOString()} />
...
export function ShipmentUpdateForm({ shipment, orderNumber }: { shipment: { id: string; status: string; carrier: string | null; tracking_number: string | null; tracking_url: string | null; package_count: number | null; estimated_delivery_at: string | null; customer_visible_note: string | null }; orderNumber: string }) {
  ...
  return <form action={action} className="mt-4 grid gap-3 sm:grid-cols-2"><input type="hidden" name="shipmentId" value={shipment.id} /><input type="hidden" name="orderNumber" value={orderNumber} />
    ...
    <label className="text-xs sm:col-span-2">Tracking URL<input name="trackingUrl" defaultValue={shipment.tracking_url ?? ""} className={`${input} mt-1`} /></label><label className="text-xs">Packages<input type="number" min="1" name="packageCount" defaultValue={shipment.package_count ?? ""} className={`${input} mt-1`} /></label><label className="text-xs">Estimated delivery<input type="datetime-local" name="estimatedDeliveryAtLocal" className={`${input} mt-1`} onChange={(e) => { const h=e.currentTarget.form?.elements.namedItem("estimatedDeliveryAt") as HTMLInputElement|null; if(h)h.value=e.currentTarget.value?new Date(e.currentTarget.value).toISOString():""; }} /><input type="hidden" name="estimatedDeliveryAt" defaultValue={shipment.estimated_delivery_at ?? ""} /></label>
