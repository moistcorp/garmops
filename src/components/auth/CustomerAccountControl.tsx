...
export default function CustomerAccountControl({ session, onOpenAuth, mobile = false, onNavigate }: { session: CustomerSession; onOpenAuth: () => void; mobile?: boolean; onNavigate?: () => void }) {
  ...
  if (mobile) return <div className="mt-2 border-t border-[var(--color-rule)] pt-3"><p className="px-1 text-xs text-black/45">Signed in as {session.email}</p><Link href="/account/orders" onClick={onNavigate} className="mt-2 block min-h-11 rounded-[4px] border border-[var(--color-rule)] px-5 py-3 text-center font-mono text-[11px] uppercase tracking-[0.06em] text-[#111111]/70">My orders</Link>{savedDesignsEnabled ? <Link href="/account/designs" onClick={onNavigate} className="mt-2 block min-h-11 rounded-[4px] border border-[var(--color-rule)] px-5 py-3 text-center font-mono text-[11px] uppercase tracking-[0.06em] text-[#111111]/70">Saved designs</Link> : null}<Link href="/account/company" onClick={onNavigate} className="mt-2 block min-h-11 rounded-[4px] border border-[var(--color-rule)] px-5 py-3 text-center font-mono text-[11px] uppercase tracking-[0.06em] text-[#111111]/70">Company details</Link><form action={logoutAction} onSubmit={onNavigate}><button className="mt-2 block min-h-11 w-full rounded-[4px] border border-[var(--color-rule)] px-5 py-3 text-center font-mono text-[11px] uppercase tracking-[0.06em] text-[#111111]/70">Log out</button></form></div>;
