...
export default function CustomerAccountControl({ session, onOpenAuth, mobile = false, onNavigate }: { session: CustomerSession; onOpenAuth: () => void; mobile?: boolean; onNavigate?: () => void }) {
  ...
  if (!session.email) return <button ref={triggerRef} type="button" onClick={onOpenAuth} className={mobile ? "mt-2 block min-h-11 w-full rounded-[4px] border border-[var(--color-rule)] px-5 py-3 text-center font-mono text-[11px] uppercase tracking-[0.06em] text-[#111111]/70" : "min-h-11 font-mono text-[11px] uppercase tracking-[0.06em] text-[#444444] transition-colors hover:text-[var(--color-accent)]"}>Login / Sign up</button>;
