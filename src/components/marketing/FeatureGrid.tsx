    ...
    <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2">
        {features.map((feature) => (
          <article key={feature.title} className="techpack-panel rounded-sm border p-5 sm:p-6">
            ...
            {feature.link && (
              <Link
                ...
                className="mt-4 inline-flex text-sm font-medium text-(--color-accent-dark) underline-offset-4 hover:underline"
              >
                {feature.link.label}
              </Link>
            )}
