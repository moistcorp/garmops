  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20">
      <div className="max-w-3xl">
        ...
        <h2 className="text-3xl font-bold tracking-tight text-[var(--text-primary)] sm:text-4xl">Choose your print finish</h2>
        <p className="mt-4 max-w-2xl text-sm leading-7 text-[#3f3f3f] sm:text-base">{introduction}</p>
      </div>

      <div className="mt-8 grid gap-4 lg:grid-cols-3">
        {techniques.map(technique => (
          <article key={technique.key} className="techpack-panel rounded-[4px] border p-5 sm:p-6">
            <h3 className="text-lg font-semibold text-[var(--text-primary)]">{technique.name}</h3>
            <p className="mt-3 text-sm leading-6 text-[#3f3f3f]">{technique.description}</p>
            <p className="mt-4 border-t border-[var(--color-rule)] pt-4 font-mono text-[10px] uppercase tracking-[0.08em] text-[var(--text-primary)]/50">
              Good for
            </p>
            <p className="mt-1 text-sm font-medium text-[var(--text-primary)]">{technique.goodFor}</p>
            {notes[technique.key] && (
              <p className="mt-4 rounded-[4px] bg-[var(--color-cream-soft)] px-4 py-3 text-xs leading-5 text-[var(--text-primary)]/65">
                {notes[technique.key]}
              </p>
            )}
          </article>
