      ...
      <ul className="mt-5 space-y-3">
        {links.map(link => (
          <li key={link.href}>
            <Link
              href={link.href}
              className="text-sm leading-6 text-(--color-accent-dark) underline decoration-(--color-accent)/35 underline-offset-4 hover:decoration-current"
            >
              {link.label}
            </Link>
          </li>
...
export default function RelatedGuides({
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20">
      <h2 className="text-3xl font-bold tracking-tight text-(--text-primary) sm:text-4xl">Continue planning</h2>
