...
export async function getCustomerOrder(supabase: OrderClient, customerUserId: string, orderNumber: string) {
  const order = await supabase.from("orders").select("id, order_number, order_type, customer_user_id, status, public_status, currency, subtotal_paise, discount_paise, taxable_value_paise, tax_paise, total_paise, amount_paid_paise, requested_delivery_date, shipping_snapshot, billing_snapshot, customer_snapshot, business_snapshot, configuration_snapshot, shipping_charge_paise, shipping_payment_status, shipping_payment_link_url, confirmed_at, created_at").eq("customer_user_id", customerUserId).eq("order_number", orderNumber).maybeSingle();
