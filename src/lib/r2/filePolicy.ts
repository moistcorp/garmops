...
type AllowedFormat = Readonly<{
  extensions: ReadonlySet<string>;
  contentTypes: ReadonlySet<string>;
}>;

type UploadPolicy = Readonly<{
  maximumBytes: number;
  formats: readonly AllowedFormat[];
}>;
...
export const uploadPolicies: Readonly<Record<BrowserUploadKind, UploadPolicy>> = {
  customer_artwork: {
    ...
    formats: [
      format(["ai", "eps"], ["application/postscript"]),
...
const uploadRequestSchema = z
  .object({
    ...
    kind: z.enum(browserUploadKinds),
...
export type UploadValidationResult =
  | { ok: true; value: ValidatedUploadRequest }
  | { ok: false; error: string };

function safeDisplayFilename(filename: string, extension: string): string {
  ...
  const safeStem = stem
    ...
    .replace(/^\.+/, "")
    .trim()
    .slice(0, Math.max(1, 240 - extension.length));

  return `${safeStem || "download"}.${extension}`;
}
...
export function validateUploadRequest(input: unknown): UploadValidationResult {
  ...
  if (
    filename === "." ||
    filename === ".." ||
    forbiddenFilenameCharacters.test(filename)
  ) {
  ...
  if (separator <= 0 || separator === filename.length - 1) {
    return { ok: false, error: "Filename extension is required" };
  }

  const extension = filename.slice(separator + 1).toLowerCase();
  const policy = uploadPolicies[parsed.data.kind];
  const allowed = policy.formats.some(
    (candidate) =>
      candidate.extensions.has(extension) &&
      candidate.contentTypes.has(parsed.data.contentType),
  );

  if (!allowed) {
    return { ok: false, error: "File type is not allowed" };
  }
  if (parsed.data.byteSize > policy.maximumBytes) {
  ...
  return {
    ...
    value: Object.freeze({
      ...
      designProjectId: parsed.data.designProjectId,
      kind: parsed.data.kind,
      visibility: parsed.data.visibility,
      originalFilename: filename,
      safeFilename: safeDisplayFilename(filename, extension),
      extension,
