...
const uploadRequestSchema = z.object({
  orderId: z.uuid().optional(), designProjectId: z.uuid().optional(), staffQuoteId: z.uuid().optional(), replacementForFileId: z.uuid().optional(), kind: z.enum(browserUploadKinds), visibility: z.enum(["customer", "staff_only"]), filename: z.string().trim().min(1).max(255), contentType: z.string().trim().toLowerCase().min(3).max(255), byteSize: z.number().int().positive(), sha256: z.string().trim().toLowerCase().regex(/^[0-9a-f]{64}$/).optional(),
}).strict().superRefine((value, context) => {
  if ([value.orderId, value.designProjectId, value.staffQuoteId].filter(Boolean).length !== 1) context.addIssue({ code: "custom", message: "Exactly one upload target is required", path: ["orderId"] });
  if (value.staffQuoteId && (value.kind !== "proof" || value.visibility !== "staff_only")) context.addIssue({ code: "custom", message: "Staff-quote uploads must be private payment proof", path: ["staffQuoteId"] });
...
export type ValidatedUploadRequest = Readonly<{ orderId?: string; designProjectId?: string; staffQuoteId?: string; replacementForFileId?: string; kind: BrowserUploadKind; visibility: "customer" | "staff_only"; originalFilename: string; safeFilename: string; extension: string; contentType: string; byteSize: number; sha256?: string }>;
