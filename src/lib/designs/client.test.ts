...
describe("cloud design artwork ownership", () => {
  ...
  it("keeps built-in sample artwork as a trusted public asset", async () => {
    const sampleUrl = "https://assets.garmops.com/garments/v5/artwork-sample.svg";
  ...
  it("re-uploads a user artwork file with only the backend upload schema fields", async () => {
    ...
    const draft = sampleDraft("https://assets.garmops.com/garments/v5/artwork-sample.svg");
    draft.artwork.front = {
      ...draft.artwork.front!,
      fileUrl: "",
