...
export async function createReservationInvoice(invoiceId: string) {
  ...
  const generated = buildInvoicePdf({ number, issuedAt, isGstInvoice, seller: { legalName: env.INVOICE_SELLER_LEGAL_NAME, address: env.INVOICE_SELLER_ADDRESS, gstin: env.INVOICE_SELLER_GSTIN, state: env.INVOICE_SELLER_STATE }, buyer: { name: organization?.legal_name || organization?.display_name || payment.customer_name, address: buyerAddress || "Address not provided", gstin: organization?.gstin, state: typeof buyerState === "string" ? buyerState : null }, lines: items.map((item) => ({ description: item.product_name, quantity: item.quantity, totalPaise: item.line_total_paise ?? Math.round(payment.amount_paise * item.quantity / Math.max(1, items.reduce((total, entry) => total + entry.quantity, 0))), hsnCode: env.INVOICE_DEFAULT_HSN_CODE })), totalPaise: payment.amount_paise, gstRateBasisPoints: env.INVOICE_GST_RATE_BASIS_POINTS });
